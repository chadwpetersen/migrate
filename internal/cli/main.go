...
const (
	...
	defaultTimezone   = "UTC"
	createUsage       = `create [-ext E] [-dir D] [-seq] [-digits N] [-format] [-tz] [-direction] NAME
	   Create a set of timestamped up/down migrations titled NAME, in directory D with extension E.
	   Use -seq option to generate sequential up/down migrations with N digits.
	   Use -format option to specify a Go time format string. Note: migrations with the same time cause "duplicate migration version" error.
	   Use -tz option to specify the timezone that will be used when generating non-sequential migrations (defaults: UTC).
	   Use -direction option to specify the creation of an up/down files only.
`
...
Database drivers: `+strings.Join(database.List(), ", ")+"\n", createUsage, gotoUsage, upUsage, downUsage, dropUsage, forceUsage)
	...
	case "create":
		...
		directionPtr := createFlagSet.String("direction", "", "The migration direction files we should create (default: create both up and down)")
		...
		if *directionPtr == "up" || *directionPtr == "down" {
			directions = []string{*directionPtr}
		}

		if err := createCmd(*dirPtr, startTime.In(timezone), *formatPtr, name, *extPtr, seq, seqDigits, directions, true); err != nil {
			log.fatalErr(err)
		}
